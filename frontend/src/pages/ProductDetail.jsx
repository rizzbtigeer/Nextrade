...
import { BRAND_NAME, WHATSAPP_LINK } from '../config.js';
...
export default function ProductDetail() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((err) => {
        ...
        if (!cancelled) setError('Impossible de charger cet article.');
      })
      .finally(() => {
  ...
  if (loading) {
    return (
      <div className="mx-auto max-w-7xl px-4 py-16 text-center text-ink-soft">
        Chargement...
      </div>
    );
  }
    ...
    return (
      <div className="mx-auto max-w-7xl px-4 py-16 text-center">
        <SEO
          ...
          description="Cet article n'est plus disponible sur NexTrade."
          url={`/produit/${slug}`}
          noindex
        />
        <p className="text-ink-soft">{error || 'Article introuvable.'}</p>
        <Link
          to="/catalogue"
  ...
  // Message WhatsApp pré-rempli pour ce produit
  const whatsappMsg = encodeURIComponent(
    `Bonjour ${BRAND_NAME}, je suis intéressé(e) par "${product.name}" (${qty} unité(s) — total estimé ${formatFCFA(total)}).`
  );
  const productWhatsAppLink = `${WHATSAPP_LINK}?text=${whatsappMsg}`;
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      ...
      <SEO
        ...
        description={
          product.description
            ...
            : `Achetez ${product.name} en gros sur ${BRAND_NAME}. Prix dégressifs, livraison rapide au Cameroun.`
        }
        image={product.images?.[0] || '/logo.png'}
        url={`/produit/${product.slug || slug}`}
        type="product"
        product={product}
        keywords={[
          product.name,
          `${product.category?.name || 'produit'} en gros`,
          `${product.category?.name || 'produit'} Cameroun`,
          `acheter ${product.name} gros`,
          BRAND_NAME,
        ]}
      />
      ...
      <Link
        to="/catalogue"
        className="mb-6 inline-flex items-center gap-1 text-sm text-ink-soft hover:text-brand"
      >
